...
## Agent Workflow
Claude project hooks run lint automatically after JS or JSX edits and block task completion if the repo lint fails. Codex should follow the same repository rule manually: after making code changes, run `npm run lint`, fix any violations, and only then finish the task.
